    ...
    <div className="bg-white/3 p-3 rounded-lg">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <button aria-label="Mes anterior" onClick={() => setView(new Date(view.getFullYear(), view.getMonth() - 1, 1))} className="w-8 h-8 flex items-center justify-center rounded-md bg-white/5 hover:bg-white/7 text-neutral-200">◀</button>
          <div className="text-white font-semibold text-lg">{view.toLocaleString(undefined, { month: 'long', year: 'numeric' })}</div>
        </div>
        <div className="flex items-center gap-2">
          <button aria-label="Mes siguiente" onClick={() => setView(new Date(view.getFullYear(), view.getMonth() + 1, 1))} className="w-8 h-8 flex items-center justify-center rounded-md bg-white/5 hover:bg-white/7 text-neutral-200">▶</button>
        </div>
      </div>

      <div className="grid grid-cols-7 gap-1 text-center text-sm">
        {['Do','Lu','Ma','Mi','Ju','Vi','Sa'].map(d => <div key={d} className="text-neutral-400 text-xs py-1">{d}</div>)}
        {days.map((d, idx) => {
